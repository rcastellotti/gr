...
./usr/local/bin/qemu-system-x86_64 \
    ...
    -drive file=./images/server-cloud-config.iso,media=cdrom,index=0 \
    -drive file=./images/sev-server.img,if=none,id=disk0,format=raw \
    -device virtio-scsi-pci,id=scsi0,disable-legacy=on,iommu_platform=true \
    -device scsi-hd,drive=disk0 \
    -device virtio-net-pci,mac=52:54:00:cc:62:02,netdev=net0,disable-legacy=on,iommu_platform=true 
    -nographic \
